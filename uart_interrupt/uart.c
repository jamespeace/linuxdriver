...
#endif
{
  ...
  if (reg & IIR_RECEIVED_AVAILABLE) {
	  printk("<1>uart_interrupt(): iir:%x, rx interrupt\n", reg);
	  if (in_irq())
		  printk("<1>execute in an interrupt handler\n");
	  else
		  printk("<1>not in interrupt handler\n");
  }
...
int __init init_module(void)
{
    ...
    // To do: PORT_COM1, 2400, BITS_8 | PARITY_NONE | STOP_ONE
	...
	if (result) {
		...
		if (prbuf)
			free_page((unsigned long)prbuf);
		if (pwbuf)
			free_page((unsigned long)pwbuf);
		release_region(PORT_COM1, 8);
		unregister_chrdev(uart_major, "uart");
		return -EFAULT;
	} else {
